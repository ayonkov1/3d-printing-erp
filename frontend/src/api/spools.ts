...
export const spoolsApi = {
    ...
    getSpools: async (): Promise<Spool[]> => {
        const response = await apiClient.get<Spool[]>('/spools')
    ...
    getSpool: async (id: string): Promise<Spool> => {
        const response = await apiClient.get<Spool>(`/spools/${id}`)
        return response.data
    },
    ...
    createSpool: async (data: SpoolCreate): Promise<Spool> => {
        const response = await apiClient.post<Spool>('/spools', data)
        return response.data
    },
    ...
    updateSpool: async (id: string, data: SpoolUpdate): Promise<Spool> => {
        const response = await apiClient.put<Spool>(`/spools/${id}`, data)
        return response.data
    },
