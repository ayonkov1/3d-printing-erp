...
export const useUpdateColor = () => {
  ...
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: Partial<ColorCreate> }) =>
      colorsApi.updateColor(id, data),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: colorKeys.lists() });
      queryClient.invalidateQueries({ queryKey: colorKeys.detail(variables.id) });
    }
  });
};
