...
export const ActionBar: React.FC<ActionBarProps> = ({ selectedAction, onActionSelect }) => {
  const actions: { id: ActionType; label: string; icon: React.ReactNode; color: string }[] = [
    { id: 'use', label: 'USE', icon: <MoreHorizontal size={28} className="text-white" />, color: 'bg-amber-500' },
    { id: 'topup', label: 'TOP UP', icon: <ChevronUp size={28} className="text-white" />, color: 'bg-lime-600' },
    { id: 'addnew', label: 'ADD NEW', icon: <Plus size={28} className="text-white" />, color: 'bg-teal-600' },
    { id: 'remove', label: 'REMOVE', icon: <X size={28} className="text-white" />, color: 'bg-red-600' },
    { id: 'data', label: 'DATA', icon: <FileText size={28} className="text-white" />, color: 'bg-purple-600' },
  ];
